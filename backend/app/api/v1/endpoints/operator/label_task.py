...
async def get_label_task(
    ...
    audit_tasks = await crud.audit_task.query(target_task_id=task.task_id).to_list()
    ...
    resp = schemas.operator.task.RespGetLabelTask(
        ...
        audit_tasks=[
            schemas.operator.task.DoTask(
                task_id=item.task_id,
                title=item.title,
                description=item.description,
                tool_config=item.tool_config,
            )
            for item in audit_tasks or []
        ],
    )
...
async def delete_label_task(
    ...
    audit_task = await crud.audit_task.query(target_task_id=req.task_id).first_or_none()
    if audit_task:
        raise exceptions.TASK_HAS_NEXT_TASK
...
async def reject_data(
    ...
    try:
        async with redis_session.lock(
            ...
            # 获取审核任务
            audit_task = await crud.audit_task.query(
            ...
            if audit_task is None:
                ...
                # 获取审核数据
                audit_datas = await crud.data.query(
                    task_id=audit_task.task_id, source_data_id=data_ids
                ).to_list()

                # data_id与audit_data_id的差集直接创建新的数据
                if audit_datas:
                    diff_data_ids = list(
                        set(data_ids).difference(
                            set([item.source_data_id for item in audit_datas])
                        )
                    )
                else:
                    diff_data_ids = data_ids
                datas = await crud.data.query(
                    task_id=req.task_id, data_id=diff_data_ids
                ).to_list()
                ...
                # data_id与audit_data_id的交集，待审核的数据直接跳过，并创建新数据
                if audit_datas:
                    query = crud.audit_flow_data.query(
                        task_id=audit_task.task_id,
                        data_id=[item.data_id for item in audit_datas],
                        status=[
                    ...
                    audit_flow_datas = await query.to_list()
                    if len(audit_flow_datas) > 0:
                        datas = await crud.data.query(
                            task_id=audit_task.task_id,
                            data_id=[item.data_id for item in audit_flow_datas],
                            status=[
                                schemas.data.DataStatus.PENDING,
                                schemas.data.DataStatus.PROCESSING,
                                schemas.data.DataStatus.DISCARDED,
                        ...
                        for data in datas:
                            ...
                            new_data_ids.append(data.source_data_id)

                        # 把审核任务的进度改为完成
                        await query.set(
                            {
                                models.audit_task.AuditFlowData.status: schemas.data.FlowDataStatus.COMPLETED
                            }
                        )  # type: ignore

                        # 把审核任务的数据状态改为废弃
                        await crud.data.query(
                            data_id=[item.data_id for item in datas]
                        ).set(
                            {
                                models.data.Data.status: schemas.data.DataStatus.DISCARDED,
                            }
                        )  # type: ignore
                        # 把审核任务的重建数据包含被打回的数据清除
                        await audit_task.update(
                            PullAll(
                                {
                                    models.audit_task.AuditTask.recreate_data_ids: [  # type: ignore
                                        item.data_id for item in datas
                                    ],
                                }
                            )
                        )
                ...
                if len(new_datas) > 0:
                    ...
                    await crud.record.query(
                        data_id=new_data_ids, user_id=req.user_id
                    ).set(
                        {
