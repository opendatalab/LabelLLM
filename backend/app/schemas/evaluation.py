...
class LabelEvaluation(BaseModel):
    # 针对单条消息的评价
    message_evaluation: dict | None = Field(description="针对单条消息的评价", default=None)

    # 针对整个对话的评价
    conversation_evaluation: dict | None = Field(description="针对整个对话的评价", default=None)
...
class AuditEvaluation(BaseModel):
    # 针对本条数据的评价
    data_evaluation: list[dict] | None = Field(description="针对本条数据的评价", default=None)
