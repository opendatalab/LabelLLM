...
class AuditRound(BaseModel):
    ...
    # 剩余审核通过次数
    remain_pass_times: int
    # 剩余审核拒绝次数
    remain_reject_times: int
...
class ReqReleaseAuditData(DoDataBase):
    flow_index: int = Field(description="流程索引")


class ReqCommitAuditData(DoDataBase):
    is_pass: bool = Field(description="数据是否通过")
    flow_index: int = Field(description="流程索引")
    data_evaluation: dict = Field(
        description="针对本条数据的评价", default_factory=dict
    )
